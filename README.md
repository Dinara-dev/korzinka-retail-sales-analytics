


# 🛒 Korzinka Retail Sales Analytics Dashboard

## 📊 Overview
Power BI dashboard analyzing retail sales performance,
product rankings, and regional revenue distribution
for Korzinka — one of Uzbekistan's largest supermarket chains.

## 🛠️ Tools Used
- Power BI Desktop
- DAX (Data Analysis Expressions)
- Excel / CSV data source

## 📌 Key Metrics
- Total Sales: 17.34 MLN so'm
- Total Quantity: 1,358
- Average Price: 12.75 K so'm
- Top Product: Cola (2.3 MLN so'm)
- Top Region: Farg'ona (3.7 MLN so'm)

## 📈 Dashboard Features
- Top purchased products ranking
- Regional sales distribution by city
- Product category breakdown (Oziq-ovqat vs Ichimlik)
- Low stock products list (Noyob mahsulotlar)
- Overall product sales trend chart

## 🗂️ Categories
- 🥗 Oziq-ovqat (Food): 60.82%
- 🥤 Ichimlik (Beverages): 39.18%

## 📷 Dashboard Preview
https://github.com/Dinara-dev/korzinka-retail-sales-analytics/blob/main/photo_2026-06-10_14-53-20.jpg

## 🗂️ Dataset
Synthetic retail dataset created for portfolio purposes.
All figures are fictional and used for demonstration only.

## 👩‍💻 Author
**Dinara Sagdullaeva**
Junior Data Analyst | Power BI Developer
📍 Tashkent, Uzbekistan
🔗 [LinkedIn](https://www.linkedin.com/in/dinara-sagdullaeva-b67740349?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app) | [GitHub](https://github.com/Dinara-dev) | [hh.uz](https://hh.uz/resume/a4989dcdff0ebf1b140039ed1f455a4e673477)
